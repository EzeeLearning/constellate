...
class db_query {
        ...
        FROM (
            ...
            FROM (
                ...
                FROM (
                    ...
                    WHERE :userid1 = -1 OR ra.contextid IN (SELECT contextid FROM {role_assignments} WHERE userid = :userid2) 
                    UNION  
                    SELECT u.id AS UserId, c.Id AS CourseId
                    FROM {user} u
                    JOIN {role_assignments} ra ON ra.userid = u.id
                    JOIN {context} ctx ON ctx.id = ra.contextid
                    JOIN {course_categories} cat ON cat.id = ctx.instanceid AND ctx.contextlevel = 40
                    JOIN {course} c ON c.category = cat.id
                    WHERE :userid3 = -1 OR ra.contextid IN (SELECT contextid FROM {role_assignments} WHERE userid = :userid4) 
                ) a
                LEFT OUTER JOIN {course_completions} cc ON cc.course = a.CourseId AND cc.userid = a.UserId
                LEFT OUTER JOIN {competency_plan} cp ON cp.userid = a.UserId
    ...
    public function stafflist($learningplan, $allstaff) {
        ...
        if ($learningplan) {
            ...
            FROM (
                ...
                FROM {user} u
                LEFT OUTER JOIN {competency_plan} p ON p.userid = u.id
                LEFT OUTER JOIN {competency_plancomp} pc ON pc.planid = p.id
                LEFT OUTER JOIN (
                    SELECT up.userid, up.planid, COUNT(uc.id) AS Completed
                    FROM {competency_usercomp} uc
                    JOIN {competency_usercompplan} up ON up.userid = uc.userid AND up.competencyid = uc.competencyid
                    WHERE uc.proficiency = 1
                    GROUP BY up.userid, up.planid
                ) ur ON ur.userid = u.id AND ur.planid = p.id
                LEFT OUTER JOIN {competency_coursecomp} cc ON cc.competencyid = pc.competencyid
                LEFT OUTER JOIN {competency_usercompcourse} ucc ON ucc.userid = u.id AND ucc.competencyid = pc.competencyid AND ucc.courseid = cc.courseid
                LEFT OUTER JOIN {logstore_standard_log} l ON l.userid = u.id
                GROUP BY u.id, p.id
            ) c
            ...
            FROM (
                ...
                JOIN {role_assignments} ra ON ra.contextid = a.ContextId
                JOIN {user} u ON u.id = ra.userid
                LEFT OUTER JOIN {course_completions} cc ON cc.course = a.CourseId AND cc.userid = u.id
                WHERE :userid1 = -1 OR ra.contextid IN (SELECT contextid FROM {role_assignments} WHERE userid = :userid2) 
                GROUP BY u.id
            ) s
            LEFT OUTER JOIN {logstore_standard_log} l ON l.userid = s.id
            GROUP BY s.Id
            ORDER BY CompletionPercentage DESC, CourseViews DESC, ModuleViews DESC, Logins DESC, LastName";
        }
    ...
    public function activitydates() {
        ...
        $DB->execute("UPDATE temp_dates SET Logins = (SELECT COUNT(*) FROM {user} WHERE FROM_UNIXTIME(lastlogin) > DATE_SUB(NOW(), INTERVAL 1 YEAR) AND MONTH(FROM_UNIXTIME(lastlogin)) = temp_dates.`Month`)", []);

        $DB->execute("UPDATE temp_dates SET Accesses = (SELECT COUNT(*) FROM {user} WHERE FROM_UNIXTIME(lastaccess) > DATE_SUB(NOW(), INTERVAL 1 YEAR) AND MONTH(FROM_UNIXTIME(lastaccess)) = temp_dates.`Month`)", []);
     ...
     */
    public function config($orderid) {
        global $DB;
